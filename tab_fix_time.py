...
from date_utils import modify_file_dates
...
class FixTimeWorker(QThread):
    ...
    def __init__(self, paths):
        ...
        self.exiftool_path = get_executable_path('exiftool.exe')
    ...
    def process_file(self, file_path):
        if not self.is_valid_filename(file_path):
            self.error.emit(f"文件名格式错误: {file_path}")
            self.move_to_error_folder(file_path)
            return

        try:
            modify_file_dates(file_path, self.exiftool_path)
        except Exception as e:
            self.error.emit(f"修复时间属性失败: {file_path}, 错误: {str(e)}")
            self.move_to_error_folder(file_path)
...
class FixTimeTab(QWidget):
    ...
    def start_processing(self):
        ...
        self.error_files = []
        self.worker = FixTimeWorker(paths)
    ...
    def processing_finished(self):
        ...
        if self.error_files:
            ...
            QMessageBox.warning(self, "处理完成", f"部分文件时间��性修复失败:\n{error_message}", QMessageBox.StandardButton.Ok)
        else:
            QMessageBox.information(self, "处理完成", "所有文件时间属性修复成功！", QMessageBox.StandardButton.Ok)
